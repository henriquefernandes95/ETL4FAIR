...
public class FairDataLoaderStepDialog extends BaseStepDialog implements StepDialogInterface {
	
	private static Class<?> PKG = FairDataLoaderStepMeta.class;

	private FairDataLoaderStepMeta input;
	...
	public FairDataLoaderStepDialog(Shell parent, Object stepMeta, TransMeta transMeta, String stepname) {
		super(parent, (BaseStepMeta) stepMeta, transMeta, stepname);

		input = (FairDataLoaderStepMeta) baseStepMeta;
		swthlp = new SwtHelper(transMeta, this.props);
		
		dialogTitle = BaseMessages.getString(PKG, "FairDataLoader.Title");
	}
	...
	public String open() {
		...
		wlStepname.setText(BaseMessages.getString(PKG, "FairDataLoader.StepNameField.Label"));
		...
		Group wGroup1 = swthlp.appendGroup(shell, lastControl, BaseMessages.getString(PKG, "FairDataLoader.Connection"));
		{
			

			wURLfair = swthlp.appendTextVarRow(wGroup1, null, BaseMessages.getString(PKG, "FairDataLoader.Connection.URL"), lsMod);

			wUserName = swthlp.appendTextVarRow(wGroup1, wURLfair, BaseMessages.getString(PKG, "FairDataLoader.Connection.User"), lsMod);

			wPassword = swthlp.appendTextVarRow(wGroup1, wUserName, BaseMessages.getString(PKG, "FairDataLoader.Connection.Password"), lsMod, true);

			wcLoadType = swthlp.appendComboVarRow(wGroup1, wPassword, BaseMessages.getString(PKG, "FairDataLoader.LoadTypeField.Label"), lsMod);
			wcLoadType.setItems(new String[] {"Catalog","Dataset","Distribution"});
			wPublish = swthlp.appendCheckboxRow(wGroup1, wcLoadType, BaseMessages.getString(PKG, "FairDataLoader.FAIRData.Publish"),
		...
		wOK.setText(BaseMessages.getString(PKG, "FairDataLoader.Btn.OK")); //$NON-NLS-1$
		wCancel = new Button(shell, SWT.PUSH);
		wCancel.setText(BaseMessages.getString(PKG, "FairDataLoader.Btn.Cancel")); //$NON-NLS-1$
	...
	protected void ok() {
		...
		try {
			...
			input.setURLfair(wURLfair.getText());//Recupera valor da variável ao abrir e fechar a caixa de diálogo
